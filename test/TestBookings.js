var Booking = artifacts.require("Bookings");
...
contract('Booking', function(accounts) {
  ...
  const price = 2;
  const weiPriceCancellable = web3.toWei(priceCancellable, "ether");
  ...
  it("should add room correctly", async () => {
    ...
    assert.equal(room[0].toNumber(), weiPriceCancellable, "Wrong cancellable price"); // Price cancellable
    assert.equal(room[1].toNumber(), web3.toWei("2", "ether"), "Wrong price"); // Price
  });

  it("should book room correctly", async () => {
    // Book the room
    const result = await instance.booking(0, checkInDate, checkOutDate, {value: weiPrice});    
  ...
  it("should revert double booking", async () => {
    ...
    try {
      await instance.booking.call(0, checkInDate, checkOutDate, {value: weiPrice});
      throw null;
    }
  ...
  it("should book with free cancellation", async () => {
    const bookingId = await instance.freeCancellation.call(0, checkInDate, checkOutDate, {value: weiPriceCancellable});
    assert.isNotNull(bookingId);
  });

  it("should be false is available for dates", async () => {
    // Book the room
    const result = await instance.freeCancellation(0, checkInDate, checkOutDate, {value: weiPriceCancellable});
    const bookingId = result.logs[0].args.bookingId;

    /// Check availability
    const isAvailable = await instance.isAvailableForDates.call(0, checkInDate, checkOutDate);
  ...
  it("should be true is available for dates", async () => {
    ...
    // Check availability
    const isAvailable = await instance.isAvailableForDates.call(0, startDate, endDate);
  ...
  it("should revert double booking with free cancellation", async () => {
    ...
    await instance.freeCancellation(0, checkInDate, checkOutDate, {value: weiPriceCancellable});
